      ...
      <>
          ...
          {data && data.map((item, itemIndex) => (
            <div key={itemIndex} className='flex  gap-2 justify-center items-center '>
              <div className='text-2xl font-bold w-80 h-80 gap-5  bg-white shadow-black shadow-2xl rounded-lg p-5 flex flex-col justify-center items-center' onClick={() => toggleBody(itemIndex)}>
                {item.title}
              </div>
              {openIndex === itemIndex && (
